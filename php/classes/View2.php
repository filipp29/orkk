 ...
 * 
 * show(_fileName_, data, return) - выводит на экран шаблон с подставленными значениями.
 ...
 *      должны совпадать с именами переменных в шаблоне. Если параметр return установлен
 *      true то вместо вывода функция вернет заполненный шаблон в виде строки 
 * 
 * --------------------------------------------------*/
/*--------------------------------------------------*/
...
//        }
    ...
    public function show(
        ...
        if (!file_exists($_file_)){
            throw new Exception("File not found ". $_fileName_. " - ". $_file);
        }
        if ($return){
            ob_start();
            require $_file_;
    ...
    public function get(
        ...
        return $this->show($_fileName_, $data, true);
    }
