...
class Model_test {
    ...
    public function getClientStatusHistory(
            $clientId
    ){
        $keyList = [
            "Подключен" => "activateDate",
            "Отключен" => "disconnectDate",
            "Переоформлен" => "renewDate"
        ];
        $result = objLoad($this->path. "{$clientId}/statusHistory");
        unset($result["#e"]);
        if (!$result){
            $params = $this->getParams($clientId);
            $clientStatus = $params["clientStatus"];
            if (in_array($clientStatus, $keyList)){
                $timeStamp = $params[$keyList[$clientStatus]] ? $params[$keyList[$clientStatus]] : $params["createDate"];
            }
            else{
                $timeStamp = $params["createDate"];
            }
            if ($timeStamp){
                $result[$timeStamp] = $params["clientStatus"];
            }
        }
        return $result;
    }
    ...
    private function saveClientStatusHistory(
        ...
        if (isset($params["clientStatus"]) && ($params["clientStatus"] != $currentClientStatus)){
            ...
            if ((count($history) == 0) && ($currentClientStatus)){
                $index = $currentParams["createDate"] ? $currentParams["createDate"] : "0";
                $history[$index] = $currentClientStatus;
            }
            if (isset($keyList[$clientStatus])){
                $key = $keyList[$clientStatus];
                if (isset($params[$key]) && $params[$key]){
                    $timeStamp = $params[$key];
                }
                else if($currentParams[$key]){
                    $timeStamp = $currentParams[$key];
                }
                foreach($history as $time => $unused){
            ...
            $history[$timeStamp] = $clientStatus;
            $path = $this->path."{$params["id"]}/statusHistory";
            objSave($path, "row", $history);
        }
    }
