...
//        $width
    ...
    if (isset($changeList["bonus"][$id])){
        $placeHolder = $changeList["bonus"][$id];
        $color = "#000080";
    }
    else{
        $placeHolder = "";
    ...
    return $view->show("inc.div",[
        ...
        "class" => "editBlock salaryBonusBlock",
        "content" => $view->get("inc.div",[
            "type" => "column",
            "content" => $view->show("inc.input.text",[
                "id" => "bonusValue",
                "class" => "salaryBonusValue",
                "style" => [
    //                "max-width" => $width,
                    "height" => "auto",
                    "width" => "100px",
    ...
    //                "min-width" => $width,
            ...
            "content" => $view->get("buttons.image",[
                ...
                "content" => $view->show("buttons.close",[
                    ...
                    "onclick" => "Salary.saveBonus(this,`td`,true)",
                    "style" => [
                        "height" => "18px"
                    ]
                ],true)
...
$getFooterTable = function()use(
    ...
    $tbody = $view->get("table.tr",[
        "content" => $view->get("table.td",[
            ...
            "content" => $bonusSum. $csvData($bonusSum). $view->get("inc.vars",[
                "vars" => $vars
            ]),
            "class" => "bonusSumBlock",
            "style" => [
...
foreach($otherBonus as $key => $value){
    $tr = [
        "name" => $key,
        "value" => $getSalaryBonusBlock($key,$value)
    ];
    $trContent = "";
    foreach($tr as $k => $val){
        if ($k == "name"){
